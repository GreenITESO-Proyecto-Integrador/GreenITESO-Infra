...
- La protección de Git `preprod` se modificó el **2026-09-25** para exigir una aprobación de PR (antes cero); se preservaron las demás opciones observadas. El workflow de migración aún no está activo en la rama protegida y esta regla no equivale a autorización de despliegue.
- El smoke ampliado del draft Backend PR30 se ejecutó en modo lectura con los roles app pooled de dev y staging el **2026-09-25**: ambas ramas reportaron 23 tablas públicas, nueve secuencias y denegación DDL. Véase [evidencia redacted](evidence/neon-app-smoke-2026-09-25.md). No es evidencia de migración por CI, Cloud Run ni production.
- Fernando confirmó el **2026-09-25** que P10 cuenta todos los registros enviados, también `REJECTED`; la implementación está en una rama local aún no fusionada. Para el catálogo compartido decidió esperar un mapeo canónico de carreras con Product/E2 antes de liberar clanes; los nombres de tres carreras investigados son candidatos, no seed autorizado.

**Pendiente humano/externo:** nombrar suplente Neon; cargar secretos reales por environment cuando estén aprobados; crear/proteger `main` en un corte separado; deshabilitar autoaprobación de `production` y acordar revisión independiente; configurar GCP e IAM; completar restore/PITR y aceptación de producción. La creación/protección de `main`, un aprobador independiente y GCP siguen siendo gates de producción. No hacer seed DRAFT en ambientes compartidos.
