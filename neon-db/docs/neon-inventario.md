...
- El trabajo local de PR33 ahora falla de forma cerrada ante alias de conexión TLS, colisiones de identidad en los seeds demo y colisiones de códigos/IDs del catálogo; revisión nativa independiente sin bloqueadores en esas guardas. PostgreSQL 18: suite enfocada de seeds **17/17** y suite completa Backend **229/229** (dos advertencias existentes); Ruff y `git diff --check` pasan. El 2026-09-24 se verificó además el recorrido aislado de devcontainer/Compose: migrations, catálogo DRAFT, demo seed, rerun idempotente y API schema HTTP 200; el stack/volumen temporal se eliminó. Backend commit `b7a0f2f` permanece local, no publicado. El fixture sigue `DRAFT`; no se cargaron filas en Neon y la ratificación de Product sigue pendiente.
...
- Verificación de solo lectura de Neon API/CLI el 2026-09-24: el proyecto conserva las ramas `production` (default/primary), `dev` y `staging`; cada rama reporta `protected=false`. El proyecto tiene `allowed_ips.ips=[]` y `protected_branches_only=false`. El schema-diff `production` → `dev` muestra 23 tablas públicas presentes en `dev` y ausentes en `production` (incluye `django_migrations`); `dev`/`staging` tienen el mismo conjunto de 23 tablas. No se aplicaron migraciones ni se escribieron datos en `production`. La rama de producción permanece sin credenciales SQL productivas verificadas; antes de cualquier release hay que aprobar y ensayar el plan inicial de migración contra PostgreSQL de producción sin asumir que el esquema ya está aplicado, verificar la aprobación de GitHub `main` y completar restore/PITR. La protección de rama Neon debe activarse desde un canal autorizado; el CLI local no expone un subcomando de protección y el conector Neon disponible no acepta la llamada por un parámetro de proyecto que falta en su esquema.
