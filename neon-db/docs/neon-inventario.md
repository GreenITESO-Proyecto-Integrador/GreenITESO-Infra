...
- El candidato local actualizado de PR30 (`b67bb97`) solo habilita la migración Neon no productiva tras un `Django tests` exitoso para un `push` protegido a `dev`/`preprod` y una PR mergeada del mismo repositorio cuyo `merge_commit_sha` coincide exactamente con el SHA probado y cuyo destino coincide con la rama. Un cierre sin merge, fork, push directo sin PR asociada, test fallido o modo Cloud no pasa el gate. Neon y Cloud Run comparten concurrencia por ambiente con `queue: max`; la elegibilidad se vuelve a revisar tras el lock, y una ejecución obsoleta se omite antes de solicitar secretos. El workflow Cloud Run permanece opt-in y es la ruta única de migración cuando se habilite. También se eliminó una interpolación de input dentro del shell del reusable workflow. Suite de contrato de release **27/27**, YAML y `git diff --check` pasan; revisión nativa independiente sin hallazgos en el fix. **No hubo ejecución de GitHub Actions y el candidato sigue local/no integrado**; no hay workflow publicado ni secreto Neon/GCP configurado. La publicación espera revisión Opus 5.5 solicitada por Fernando; Claude Code estaba sin sesión autenticada al 2026-09-24.
